...
// Определяем base path надёжно — работает и на localhost и на GitHub Pages
const APP_BASE = (() => {
  // Если есть <base href>, используем его
  const baseEl = document.querySelector('base[href]');
  if (baseEl) {
    const b = baseEl.getAttribute('href');
    return b.endsWith('/') ? b : `${b}/`;
  }
  // Иначе берём из window.location: всё до последнего /
  // Например: https://4erfox.github.io/milana123book/index.html → /milana123book/
...
const withBase = (relativePath) =>
  `${APP_BASE}${String(relativePath).replace(/^\/+/, '')}`;

// ─── Загрузка страниц + содержимого MD ───────────────────────────────────────

async function loadPages() {
  try {
    const res = await fetch(withBase('data/nav.json'));
    if (!res.ok) throw new Error(`nav.json: ${res.status}`);
    const nav = await res.json();
    const pages = nav.flatMap(section =>
      section.pages.map(p => ({
        title:   p.title,
        href:    withBase(String(p.href || '').replace(/^\/+/, '')),
    ...
    // Догружаем MD контент в фоне для полнотекстового поиска
    Promise.allSettled(
      pages.map(async p => {
        try {
          const r = await fetch(withBase(`docs/${p.slug}.md`));
          if (!r.ok) return;
    ...
    console.warn('[search] не удалось загрузить nav.json:', e.message);
    allPages = [];
  }
}
...
// Кнопка поиска — ищем по id="search-btn" ИЛИ вторую кнопку в .bottom-nav
